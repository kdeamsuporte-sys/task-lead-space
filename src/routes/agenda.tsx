      ...
      <div className="space-y-6">
        ...
        <div className="glass-card rounded-3xl p-4 md:p-6">
          <div className="mb-4 flex items-baseline justify-between">
            <div>
              <div className="text-xs font-bold uppercase tracking-[0.18em] text-primary/80">28 de Março · Sexta-feira</div>
              <div className="mt-1 text-lg font-bold">{appointments.length} compromissos</div>
            </div>
          ...
          <ol className="relative space-y-4 border-l-2 border-border-soft pl-6">
            {appointments.map((a) => {
              const tone = a.status === "atrasado" ? "danger" : a.status === "pendente" ? "warning" : "success";
              return (
                <li key={a.id} className="relative">
                  <span className={`absolute -left-[31px] top-1 h-4 w-4 rounded-full ring-4 ring-background ${
                    a.status === "atrasado" ? "bg-destructive" : a.status === "pendente" ? "bg-warning" : "bg-success"
                  } shadow-[0_0_0_3px_oklch(0.72_0.205_38_/_0.15)]`} />

                  <div className="glass-card ring-premium group rounded-2xl p-4 transition hover:-translate-y-0.5 hover:border-primary/40">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1.5 text-base font-bold">{a.client}</div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button className="flex items-center gap-1.5 rounded-lg bg-success/15 px-3 py-2 text-xs font-bold text-success ring-1 ring-success/30 min-h-[44px]"><MessageCircle className="h-3.5 w-3.5" /> WhatsApp</button>
                        <button className="flex items-center gap-1.5 rounded-lg border border-border bg-background/60 px-3 py-2 text-xs font-bold min-h-[44px]"><Check className="h-3.5 w-3.5" /> Confirmar</button>
                        <button className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-background/60 min-h-[44px]"><RotateCcw className="h-3.5 w-3.5" /></button>
                        <button className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-background/60 min-h-[44px]"><Phone className="h-3.5 w-3.5" /></button>
                      </div>
                    </div>
                  </div>
                </li>
