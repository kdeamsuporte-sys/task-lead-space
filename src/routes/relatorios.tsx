      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <section className="glass-card rounded-3xl p-5">
            <div className="mb-4">
              <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary/80">Funil comercial</div>
              <h3 className="mt-1 text-lg font-bold">Conversão por etapa</h3>
            </div>
            <div className="space-y-2">
              {reports.funnel.map((f, i) => (
                <div key={f.stage} className="relative">
                  <div
                    className="relative flex items-center justify-between overflow-hidden rounded-xl border border-primary/20 px-4 py-3"
                    style={{
                      width: `${100 - i * 4}%`,
                      background: `linear-gradient(90deg, oklch(0.72 0.205 38 / ${0.18 - i * 0.025}), oklch(0.72 0.205 38 / ${0.05}))`,
                    }}
                  >
                    <span className="text-sm font-bold">{f.stage}</span>
          ...
          <section className="glass-card rounded-3xl p-5">
            <div className="mb-4">
              <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary/80">Origem dos leads</div>
              <h3 className="mt-1 text-lg font-bold">Canal que mais converte</h3>
            </div>
          ...
          <section className="glass-card rounded-3xl p-5 lg:col-span-2">
            <div className="mb-4 flex items-baseline justify-between">
              <div>
                <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary/80">Motivos de perda</div>
                <h3 className="mt-1 text-lg font-bold">Por que estamos perdendo?</h3>
              </div>
              <span className="text-xs text-muted-foreground">% das oportunidades perdidas</span>
            </div>
            <div className="grid gap-2 md:grid-cols-5">
              {reports.lostReasons.map((r) => (
                <div key={r.label} className="rounded-2xl border border-border-soft bg-background/40 p-3">
                  ...
                  <div className={cn("mt-1 text-2xl font-black tabular-nums", r.pct >= 30 ? "text-destructive" : r.pct >= 15 ? "text-warning" : "text-foreground")}>{r.pct}%</div>
